# 🐍 Snake Game em Tkinter - Versão Orientada a Objetos com SOLID

Versão completa do jogo Snake usando Tkinter, mantendo a arquitetura OO com princípios SOLID da versão anterior.
...
## **📊 Comparação: Curses vs. Tkinter**

| **Aspecto** | **Curses (Terminal)** | **Tkinter (GUI)** |
|------------|---------------------|------------------|
| **Plataforma** | Linux/macOS nativo | Windows/Linux/macOS |
| **Instalação** | stdlib (Unix) | stdlib (todas) |
| **Gráficos** | Caracteres ASCII | Canvas com formas |
| **Cores** | Limitadas (8-256) | RGB completo |
| **Input** | `getch()` não-bloqueante | Event-driven (bind) |
| **Game Loop** | `time.sleep()` controle | `after()` recursivo |
| **Linhas de código** | ~650 | ~720 |
| **Performance** | Mais leve | Um pouco mais pesado |

---

## **🏗️ Arquitetura Mantida (SOLID)**
...
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                  SnakeGame (Facade)                     │    │
│  │  - _game_loop(), _update_game_logic(), callbacks        │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
|----------|---------|
| `↑` `↓` `←` `→` | Mover cobra |
| `W` `A` `S` `D` | Mover cobra (alternativo) |
| `P` | Pausar/Retomar |
| `R` | Reiniciar (após Game Over) |
| `Q` ou `Escape` | Sair do jogo |

---

## **🎨 Recursos Visuais**

1. **Cobra com olhos** que seguem a direção do movimento
2. **Grid sutil** no fundo para referência visual
3. **HUD informativo** com score, velocidade e status
4. **Cores modernas** (tema escuro)
5. **Telas de Start e Game Over** centralizadas
...
```bash
# Salve o código como snake_tkinter.py
...
| **Vantagem** | **Descrição** |
|-------------|--------------|
| **Multiplataforma** | Funciona nativamente no Windows |
| **Gráficos melhores** | Canvas com formas, cores RGB |
| **Mais acessível** | Não requer terminal configurado |
| **Fácil estender** | Adicionar botões, menus, imagens |
| **Event-driven** | Modelo nativo do Tkinter |

---

## **🔧 Possíveis Extensões**

1. **High Score persistente**: Salvar em arquivo JSON
2. **Sons**: Usar `pygame.mixer` ou `winsound`
3. **Temas**: Permitir troca de cores/estilos
4. **Dificuldades**: Fácil, Médio, Difícil
5. **Power-ups**: Comidas especiais com efeitos
6. **Modo 2 jogadores**: Cobra vs Cobra local

Esta versão mantém toda a arquitetura OO e SOLID da versão curses, adaptando apenas as camadas de infraestrutura (Renderer e InputHandler) para o ecossistema Tkinter! 🎮